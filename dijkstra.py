...
class VPNFileTransferApp:
    ...
    def __init__(self, root):
        ...
        # Variables de control para la lógica de la aplicación
    ...
    # --- MÉTODOS DE LÓGICA PRINCIPAL Y SIMULACIÓN ---
    ...
    def create_widgets(self):
        ...
        log_frame = ttk.LabelFrame(main_frame, text="Registro de Simulación", padding="10")
    ...
    def log_message(self, message):
        if self.root.winfo_exists() and self.log_text.winfo_exists():
            self.log_text.insert(tk.END, message + "\n")
            self.log_text.see(tk.END)
            self.root.update_idletasks()

    # Muestra el numero activo de simulaciones 
    def update_transfer_counter_display(self):
        if self.root.winfo_exists() and self.transfer_counter_label.winfo_exists():
    ...
    def update_progress_display(self, value):
        if self.root.winfo_exists() and self.progress.winfo_exists():
...
if __name__ == "__main__":
    ...
    app = VPNFileTransferApp(root)  # Crea una instancia de la aplicación
